...
class Residual_CNN(Gen_Model):
	...
	def value_head(self, x):
		# Calculate the value of a position (Prob to win from that position)
	...
	def _build_model(self):
		...
		### Add the three HEADs at the top of the BODY
		vh = self.value_head(x)
		mh = self.move_head(x)
		th = self.tile_head(x)

		model = Model(inputs=[main_input], outputs=[vh, mh, th])
		model.compile(loss={'value_head': 'mean_squared_error', 'move_head': softmax_cross_entropy_with_logits, 'tile_head': softmax_cross_entropy_with_logits},
			optimizer=SGD(lr=self.learning_rate, momentum = config.MOMENTUM),	
			loss_weights={'value_head': 0.5, 'move_head': 0.5, 'tile_head': 0.5}
			)
